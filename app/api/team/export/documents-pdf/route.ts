...
function generatePDF(
  ...
  violationsWithDocs.forEach((violation, index) => {
    ...
    doc.setTextColor(orange);
    doc.setFont('helvetica', 'bold');
    doc.text(`${index + 1}. ASIN: ${violation.asin}`, margin, yPos);
    yPos += 7;
  ...
  yPos += 5;
  doc.setFont('helvetica', 'normal');
  doc.text('Send documents to: info@sellercentry.com', margin + 5, yPos);
  yPos += 8;

  // Option 3: Reply to email
  doc.setFont('helvetica', 'bold');
  doc.text(documentFolderUrl ? 'Option 3: Reply to This Email' : 'Option 2: Reply to This Email', margin, yPos);
  yPos += 5;
  doc.setFont('helvetica', 'normal');
  doc.text('Attach your documents directly to your email reply.', margin + 5, yPos);
  yPos += 10;
  ...
  yPos += 4;
  doc.text('sellercentry.com | info@sellercentry.com', pageWidth / 2, yPos, { align: 'center' });
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!isTeamMember(user.email)) {
      return NextResponse.json({ success: false, error: 'Access denied - not a team member' }, { status: 403 });
    }
    ...
    // Fetch active violations with docs needed
    const violations = await getViolationsForTeam(tenant, 'active');
    ...
    if (violationsWithDocs.length === 0) {
      return NextResponse.json({
        success: false,
        error: 'No documents are currently requested for this client'
      }, { status: 400 });
    }
    ...
    const filename = `SellerCentry_DocsNeeded_${sanitizedStoreName}_${dateStr}.pdf`;

    console.log(`[GET /api/team/export/documents-pdf] Generated PDF for ${subdomain} with ${violationsWithDocs.length} violations`);
